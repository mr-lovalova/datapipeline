...
def validate_dataset_feature_identity(
    ...
    for cfg in [*dataset.features, *dataset.targets]:
        stream = require_runtime_stream(runtime, cfg.stream)
        if not stream.partition_by:
            continue
        if stream.feature_id_by is None:
            raise ValueError(
                f"Stream '{cfg.stream}' has partition_by and is used as a "
        ...
        if missing_partition_fields:
            raise ValueError(
                f"Stream '{cfg.stream}' does not preserve partition identity "
