...
class VectorDropVerticalTransform(VectorContextMixin):
    ...
    Requires the `build/metadata.json` artifact generated by the
    `metadata` build task. The transform evaluates coverage using the recorded
    `present_count`/`null_count` metrics and prunes the effective schema so
    ...
    def apply(self, stream: Iterator[Sample]) -> Iterator[Sample]:
        ...
        for sample in stream:
            ...
            if vector is None or not vector.values:
                yield sample
                continue
            retained: dict[str, float | list[float]] = {}
            changed = False
    ...
    def _maybe_prune_schema(self, drop_ids: set[str]) -> None:
        if self._schema_pruned:
            return
        context = self._context or try_get_current_context()
        if not context:
            raise RuntimeError(
                "VectorDropVerticalTransform requires an active pipeline context."
            )
        context.remove_schema_ids(self._payload, drop_ids)
