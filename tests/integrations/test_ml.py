...
def _register_entry_points(monkeypatch):
    ...
    monkeypatch.setattr("datapipeline.services.factories.load_ep", _fake_load_ep)
    monkeypatch.setattr("datapipeline.pipeline.utils.transform_utils.load_ep", _fake_load_ep)
...
def test_iter_vector_rows_flatten_sequences(tmp_path):
    ...
    rows = list(iter_vector_rows(project_yaml, limit=1, flatten_sequences=True))
