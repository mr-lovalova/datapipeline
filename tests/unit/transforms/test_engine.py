...
def _non_closing_function(stream: Iterator[Any]) -> Iterator[Any]:
    return _NonClosingIterator(stream)


class _NonClosingTransform:
...
@pytest.mark.parametrize(
    "entrypoint",
    [
        pytest.param(_non_closing_function, id="function"),
        pytest.param(_NonClosingTransform, id="class"),
    ],
)
def test_apply_transforms_closes_upstream_for_non_closing_entrypoint(
    monkeypatch,
    entrypoint,
) -> None:
    closed: list[str] = []
